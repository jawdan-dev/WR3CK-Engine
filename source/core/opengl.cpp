...
const size_t getTypeBase(const GLenum glType) {
	...
	WR3CK_ERROR("Unhandled GL type \"0x%x\".", glType);
	return 0;
}
const size_t getTypeBaseByteSize(const GLenum glType) {
	// Primitives only. :)
	switch (getTypeBase(glType)) {
		...
		// case GL_TEXTURE:
		// case GL_SAMPLER: return sizeof(GLuint); // TODO: ?
	}
	WR3CK_ERROR("Unhandled GL type \"0x%x\".", glType);
	return 0;
}
...
const size_t getTypeVertexSplitCount(const GLenum glType) {
	// TODO: I'm not sure whether the first or last is the determining value.
	switch (glType) {
		// 2
		case GL_FLOAT_MAT2:
		case GL_DOUBLE_MAT2:
...
const size_t getTypeByteSize(const GLenum glType) {
	return getTypeBaseByteSize(glType) * getTypeElementCount(glType);
}
}